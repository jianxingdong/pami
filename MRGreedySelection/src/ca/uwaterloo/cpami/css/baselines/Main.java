...
public class Main {
	 ...
	 * 
	 * PrintWriter pw = new PrintWriter(fs.create( new Path("/results/CSS.txt"),
	 * true));
	 * 
	 * String tmpCSS = "/tmp/css/tcc"; String tmpMatC = "/tmp/Ctmp/tc";
	 * 
	 * int numPartitions = 800; String cssNewMatrix = "greedycss/C"; String
	 * cssNewMatrixOrth = "greedycss/COrth"; float l = 0.2f;
	 * 
	 * for (int k : ks) { System.out.println(k); fs.delete(new Path(tmpCSS),
	 * true); fs.delete(new Path(tmpMatC), true); fs.delete(new
	 * Path(cssNewMatrix), true); long time = System.currentTimeMillis();
	 * Driver.main(new String[] { A.getRowPath().toString(), tmpCSS,
	 * numPartitions + "", cssNewMatrix, numCols + "", k + "", l + "" }); long
	 * duration = System.currentTimeMillis() - time; // Orthogonalization
	 * CentralizedOrthogonalization.orthonormalize(new Path(cssNewMatrix),
	 * numRows, k, new Path(cssNewMatrixOrth)); DistributedRowMatrix C = new
	 * DistributedRowMatrix(new Path( cssNewMatrixOrth), new Path(tmpMatC),
	 * numRows, k); C.setConf(config);
	 * 
	 * double recErr = new ReconstructionError().clacReconstructionErr(A, C,
	 * numReducers); pw.println(k + "\t" + duration + "\t" + recErr);
	 * pw.flush(); } pw.close(); }
	 */
	 ...
	 * InterruptedException, ClassNotFoundException {
	 * 
	 * String ssvdOutput = "ssvd/output"; int aBlockRows = 10000; PrintWriter pw
	 * = new PrintWriter(fs.create( new Path("/results/SSVD.txt"), true));
	 * 
	 * String tmpMatC = "/tmp/Ctmp"; for (int k : ks) { fs.delete(new
	 * Path(tmpMatC), true); fs.delete(new Path(ssvdOutput), true); SSVDSolver
	 * ssvdSolver = new SSVDSolver(config, new Path[] { A.getRowPath() }, new
	 * Path(ssvdOutput), aBlockRows, k, (int) (0.1 * k), numReducers);
	 * ssvdSolver.setQ(1); long time = System.currentTimeMillis();
	 * ssvdSolver.run(); long duration = System.currentTimeMillis() - time;
	 * DistributedRowMatrix C = new DistributedRowMatrix(new Path(
	 * ssvdSolver.getUPath()), new Path(tmpMatC), numRows, k);
	 * C.setConf(config); double recErr = new
	 * ReconstructionError().clacReconstructionErr(A, C, Main.numReducers);
	 * pw.println(k + "\t" + duration + "\t" + recErr); pw.flush(); }
	 * pw.close(); }
	 */
	...
	public static void main(String[] args) throws IOException,
		...
		/*
		DistributedRowMatrix A = new DistributedRowMatrix(
				new Path("s3n://greedycss/pubmed-doc-word.dat"), new Path(tmpMatA), 8200000, 141043);
		A.setConf(config);
		A = A.transpose();
		
		new FirstKColumnsSelectionJob().selectFirstKCols(A.getRowPath().toString(), 200000,
				"/common/A");
				*/
